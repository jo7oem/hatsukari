...
func Load(file io.Reader) error {
	cnfBuf, err := io.ReadAll(file)
	if err != nil {
		return err
	}

	var cConf contentsConf

	if err := yaml.Unmarshal(cnfBuf, &cConf); err != nil {
		return err
	}

	if cConf.About.Name == "" || cConf.About.Path == "" {
		return errors.New("invalid contents config")
	}
